...
//
//  ------------------------------------------------------------------
//  作者：Keneth Wagner//  最后修改：2022-07-06
//
//  Language: C# 1.0
//  ------------------------------------------------------------------
//
...
namespace Peak.Can.Basic
{
    ...
    {
        /// <summary>
        /// 未定义、未知或未选择的 PCAN 设备值
        /// </summary>
        PCAN_NONE = 0,
        /// <summary>
        /// PCAN 非即插即用设备。在 PCAN-Basic API 中未使用
        /// </summary>
        PCAN_PEAKCAN = 1,
        /// <summary>
        /// PCAN-ISA、PCAN-PC/104 和 PCAN-PC/104-Plus
        /// </summary>
        PCAN_ISA = 2,
        /// <summary>
        /// PCAN-Dongle
        ...
        PCAN_PCC = 6,
        /// <summary>
        /// PCAN 虚拟硬件。在 PCAN-Basic API 中不使用
        /// </summary>
        PCAN_VIRTUAL = 7,
        /// <summary>
        /// PCAN 网关设备
    ...
    public static class PCANBasic
    {
       ...
       /// <param name="Btr0Btr1">通信速度 (BTR0BTR1 码)</param>
       /// <param name="HwType">非即插即用: 硬件类型和操作模式</param>
       ...
       ///  <returns>一个 TPCANStatus 错误代码</returns>
        ...
        /// </summary>
      /// <param name="Channel">PCAN 通道的句柄</param>
      /// <param name="Btr0Btr1">通信速度 (BTR0BTR1 代码)</param>
      /// <returns>返回一个 TPCANStatus 错误代码</returns>     
        ...
        /// 初始化一个 PCAN FD 通道
        /// </summary>
        ///<param name="Channel">支持 FD 的 PCAN 通道的句柄</param>
        ///<param name="BitrateFD">通信速度（FD 比特率字符串）</param>
        ///<remarks> 请参阅 PCAN_BR_*值
        ///比特率字符串必须遵循以下构造规则：
        ///*参数和值必须用“=”分隔
        ///*参数/值对必须用 ',' 分隔
        ...
        /// <summary>
        ///  取消由 CAN_Initialize 函数初始化的单个或所有 PCAN 通道。
        /// </summary>
        /// <remarks>如果给定 TPCANHandle 值 "PCAN_NONEBUS"，
      ...
      /// <returns>一个 TPCANStatus 错误代码</returns>
        ...
        /// <summary>
        /// 从支持 PCAN FD 功能的通道的接收队列中读取 CAN 消息
        /// </summary>
        ///  <param name="Channel">支持 FD 的 PCAN 通道的句柄</param>
        ...
        /// 从支持 PCAN FD 功能的通道的接收队列中读取 CAN 消息
        /// </summary> 
        /// /<param name="Channel">支持 FD 的 PCAN 通道句柄</param>
        /// <param name="MessageBuffer">用于存储 CAN 消息的 TPCANMsgFD 结构缓冲区</param> 
        /// <returns>TPCANStatus 错误代码</returns>
        ...
        ///  配置接收滤波器
        ///  </summary>
        /// <remarks>每次调用此函数时，消息过滤器都会扩展。如果希望重置过滤器，请使用 'SetValue' 函数</remarks>
        /// <param name="Channel">PCAN通道的句柄</param>
        /// <param name="FromID">要接收的最低CAN ID</param>
        ...
        /// 获取一个 PCAN 通道的值
        ///  </summary>
        ///  <remarks>参数可能存在也可能不存在，这取决于所使用的硬件类型（PCAN 通道）。如果参数不可用，将返回 PCAN_ERROR_ILLPARAMTYPE 错误</remarks>
        ///  <param name="Channel">PCAN 通道的句柄</param>
        ///  <param name="Parameter">要获取的 TPCANParameter 参数</param>
        ...
        ///  </summary>
        ///  <remarks>
        /// 参数可能存在也可能不存在，这取决于所使用的硬件类型（PCAN 通道）。如果参数不可用，将返回 PCAN_ERROR_ILLPARAMTYPE 错误
        /// </remarks>
        ///  <param name="Channel">PCAN 通道的句柄</param>
        /// <param name="Parameter">要获取的 TPCANParameter 参数</param>
        ...
        /// 获取 PCAN 通道的值
        /// </summary>
        /// <remarks>根据所使用的硬件（PCAN 通道）的类型，参数可能存在也可能不存在。如果参数不可用，将返回 PCAN_ERROR_ILLPARAMTYPE 错误
        /// </remarks>
        /// <param name="Channel">PCAN 通道的句柄</param>
        /// <param name="Parameter">要获取的 TPCANParameter 参数</param>
        /// <param name="ChannelsBuffer">用于存放参数值的缓冲区</param>
        /// <returns>TPCANStatus 错误码</returns>
        ...
        /// 配置或设置 PCAN 通道的值
        /// </summary> 
        /// <remarks>根据所使用的硬件（PCAN 通道）类型，参数可能存在也可能不存在。如果参数不可用，        
        /// 将返回 PCAN_ERROR_ILLPARAMTYPE 错误</remarks> 
        /// <param name="Channel">PCAN 通道的句柄</param> 
        /// <param name="Parameter">要设置的 TPCANParameter 参数</param>
        ...
        /// 配置或设置 PCAN 通道的值
        /// </summary> 
        /// <remarks>根据所使用的硬件（PCAN 通道）类型，参数可能存在也可能不存在。如果参数不可用，将返回 PCAN_ERROR_ILLPARAMTYPE 错误</remarks>
        /// <param name="Channel">PCAN 通道的句柄</param> 
        /// <param name="Parameter">要设置的 TPCANParameter 参数</param>
        ...
        /// 配置或设置一个 PCAN 通道的值       
        /// </summary> 
        ///<remarks>根据所使用的硬件（PCAN 通道），参数可能存在也可能不存在。如果参数不可用，将返回 PCAN_ERROR_ILLPARAMTYPE 错误</remarks>       
        ///<param name="Channel">PCAN 通道的句柄</param> 
        ...
        /// <summary>
        /// 返回给定 TPCANStatus 错误代码的描述文本，可使用任意所需语言/
        /// </summary>/
        /// <remarks>当前可用的翻译语言包括：/// 中性 (0x00)、德语 (0x07)、英语 (0x09)、西班牙语 (0x0A)、/// 意大利语 (0x10) 和法语 (0x0C)</remarks>
